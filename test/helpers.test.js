...
await describe('VoterViewApi/utilities', async () => {
    await it.skip('should parse a JSON Date with a time offset correctly', () => {
        ...
        assert.strictEqual(parsedDate?.toISOString(), expectedDate.toISOString(), 'The parsed date should match the expected UTC date');
    });
    await it('should parse a variety of date formats correctly', () => {
        ...
        for (const { expected, input } of testCases) {
            ...
            if (expected === undefined) {
                assert.strictEqual(parsedDate, undefined, `Expected undefined for input: ${input}`);
            }
            else {
                assert.strictEqual(parsedDate?.getUTCFullYear(), expected.year, `The parsed date should match the expected year for input: ${input}`);
                assert.strictEqual(parsedDate.getUTCMonth() + 1, expected.month, `The parsed date should match the expected month for input: ${input}`);
                assert.strictEqual(parsedDate.getUTCDate(), expected.day, `The parsed date should match the expected day for input: ${input}`);
            }
        }
    });
});
